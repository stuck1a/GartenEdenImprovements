...
--
-- Funktion für das korrekte Erzeugen des ISO-Objekts.
-- Wird bei Fertigstellung eines Bauprojekts aufgerufen,
--
BCCrafTecTA.worldCraftingFinished = function(object, recipe, character, retVal)
  if retVal.object then return end;
  ...
  if md.resultClass == "ISLightSource" then
    -- Deltawerte für Lichtquellen von Fackel übernehmen zwecks Kompatibilität
    ...
    -- Konstruktor-Calls müssen wegen den unterschiedlichen Parametern gemappt werden
...
--
-- Zusätzliche Hilfsfunktion für den Fertigstellungs-Prozess.
-- Einige ISO-Objekte müssen funktionell initialisiert werden, was hier geschieht.
-- Aktuell werden Türschlüssel mit den Türen verknüpft und Regentonnen gefixt.
--
...
--
-- Util-Funktion.
-- Unserialisiert gespeicherte ModData von einem ISO-Objekt (bzw. allg. Java-Objekt)
-- und erzeugt daraus eine Lua-Tabelle.
--
...
--
-- Util-Funktion.
-- Überführt gespeicherte ModData von einem ISO-Objekt (bzw. allg. Java-Objekt)
-- in die Form, die für die Bauprojekt-Objekte benötigt werden.
--
local copyModData = function(javaObject, dst)
  local md = javaObject:getModData();
  local dmd = dst.javaObject:getModData();
  -- Benötigte Materialen aus ModData ableiten
  ...
  -- Sprite-Namen aus ModData ableiten
  dst:getSprite();
end
...
--
-- Funktion, die den gespeicherten Zustand eines Baustelle-Objekts wiederherstellt, indem sie die zugeordneten
-- ModData-Infos mit entsprechenden Funktionsaufrufen der Instanz verknüpft.
--
...
--
-- Überprüft, ob das Bauprojekt abgeschlossen ist.
-- Wird nach jedem Baufortschritt aufgerufen.
--
function BCCrafTecTA:checkIfFinished()
  if not self.object then return end
  for skills in self.recipe.requirements do
    for s in skills do
      if self.isDeconstruction then
...
--
-- Berechnet die Dauer der TimedAction und damit die Bauzeit des Bauprojekts
--
function BCCrafTecTA:calculateMaxTime()
  ...
  retVal = retVal / f;
  retVal = retVal + 2 / f; -- Zeit minimal erhöhen, damit es nicht exakt zwei Minuten sind
  return retVal;
end
...
--
-- Util-Funktion, um die korrekten Sprites je nach Skill-Level zu ermitteln
-- Vermutlich deprecated, da ausgelagert
--
...
--
-- Konstruktor für die TimedAction-Instanz.
-- Überprüft, ob alle Bedingungen für den Start der TA gegeben sind (Werkzeuge etc)
-- und ermittelt dann die Gesamtdauer (ggf. Restdauer) der Aktion.
--
function BCCrafTecTA:new(character, object, isDeconstruction)
  local modData = object:getModData();
  if not modData.recipe then
    print("[CrafTec] ERROR: modData.recipe is missing");
    return false;
  end
  local o = {}
  setmetatable(o, self)
