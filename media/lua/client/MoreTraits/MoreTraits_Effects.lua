...
MoreTraits.settings = MoreTraits.settings or {
  ScroungerAnnounce = false,
  ScroungerHighlight = true,
  ScroungerHighlightTime = 20,
  GraveRobberAnnounce = false,
  SuperImmuneAnnounce = false,
  GourmandAnnounce = false,
  AlbinoAnnounce = true,
  MartialDamage = false,
  VagabondAnnounce = false,
  HardyNotifier = false
}
...
function ZombificationCure_OnCreate(items, result, player)
  ...
  for i = bodyParts:size() - 1, 0, -1 do
    ...
    if bodyPart:IsInfected() then bodyPart:RestoreToFullHealth() end
  end
...
function initToadTraitsItems(_player)
  ...
  if player:HasTrait("drinker") then
    if SandboxVars.MoreTraits.AlcoholicFreeDrink == true then inv:addItemOnServer(inv:AddItem("Base.WhiskeyFull")) end
  end
  ...
  if player:HasTrait("deprived") then
    ...
    if SandboxVars.MoreTraits.ForgivingDeprived == true then inv:addItemOnServer(inv:AddItem("Base.Belt2")) end
  end
end
...
function ToadTraitAntique(_iSInventoryPage, _state, _player)
  ...
  local length = 0
  for k, v in pairs(items) do length = length + 1 end
  local player = _player;
  local playerdata = player:getModData();
  local containerObj;
  local container;
...
function graveRobber(_zombie)
  ...
  local chance = 10;
  local extraloot = 1;
  if SandboxVars.MoreTraits.GraveRobberChance then chance = SandboxVars.MoreTraits.GraveRobberChance end
  if SandboxVars.MoreTraits.GraveRobberGuaranteedLoot then extraloot = SandboxVars.MoreTraits.GraveRobberGuaranteedLoot end
...
function setFoodState(food, state)
  ...
  if curState ~= nil and curHungChange ~= nil then
    ...
    if curHungChange ~= oldHungChange or curSpices ~= oldSpices then curState = nil end
  end
  if curState == nil then
    local comparativechange = 0;
    if food:isFrozen() == true then comparativechange = comparativechange + 30 end
    if food:isRotten() == true then comparativechange = comparativechange + 10 end
    if food:isFresh() == false then comparativechange = comparativechange + 10 end
    ...
    if itemdata.iFoodStage == nil then itemdata.iFoodStage = 0 end
...
local function MotionSickness(player)
  ...
  if playerdata.MotionActive == nil then playerdata.MotionActive = false end
  if player:HasTrait("motionsickness") then
    if player:isDriving() == true and Sickness < 90.0 then
      local vehicle = player:getVehicle();
      if not vehicle then return end
      if playerdata.MotionActive == false then playerdata.MotionActive = true end
    ...
    elseif not player:isDriving() and not playerstats:IsFakeInfected() and Sickness ~= 0 then
      if playerdata.MotionActive == true then playerdata.MotionActive = false end
      playerstats:setFakeInfectionLevel(Sickness - 0.1);
    end
  end
end
...
local function RestfulSleeperWakeUp(player, playerdata)
  ...
  elseif player:HasTrait("restfulsleeper") and playerdata.HasSlept == true then
    if Fatigue > playerdata.FatigueWhenSleeping then player:getStats():setFatigue(playerdata.FatigueWhenSleeping) end
    playerdata.HasSlept = false;
    playerdata.FatigueWhenSleeping = 0;
    if playerdata.NeckHadPain == false and Neck:getAdditionalPain() > 0 then Neck:setAdditionalPain(0) end
  end
end
...
local function HungerCheck(player)
  ...
  if player:HasTrait("SuperImmune") and player:getModData().SuperImmuneActive == true then
    ...
    if player:getModData().SuperImmuneMinutesWellFed == nil then player:getModData().SuperImmuneMinutesWellFed = 0 end
    if hunger == 0 then player:getModData().SuperImmuneMinutesWellFed = SuperImmuneMinutesWellFed + 1 end
...
local function ImmunocompromisedInfection(player, playerdata)
  ...
  local chance = 25;
  local checked = false;
  if SandboxVars.MoreTraits.ImmunoChance then chance = SandboxVars.MoreTraits.ImmunoChance end
  if evasivecheck == nil then playerdata.ImmunoPart = {} end
  if activated == nil then playerdata.ImmunoActivated = false end
  if playerdata.ImmunoFinal == nil then playerdata.ImmunoFinal = false end
  if playerdata.ImmunoEvasiveTimer == nil then playerdata.ImmunoEvasiveTimer = 0 end
  if activated == true then
    if player:getCurrentState() == IdleState.instance() then playerdata.ImmunoActivated = false end
  end
  if playerdata.ImmunoFinal == true and isinfected == false then playerdata.ImmunoFinal = false end
  if player:HasTrait("Immunocompromised") then
  ...
  if player:getPrimaryHandItem() ~= nil then
    if player:getPrimaryHandItem():getCategory() == "Weapon" then
      if player:getPrimaryHandItem():getSubCategory() == "Firearm" then
        ...
        if itemdata.MTstate == nil then itemdata.MTstate = "Normal" end
...
function MainPlayerUpdate(_player)
  ...
  local playerdata = player:getModData();
  if internalTick >= 30 then
    amputee(player, (playerdata.bWasInfected ~= player:getBodyDamage():isInfected() and player:getBodyDamage():isInfected()));
  ...
  if internalTick > 30 then internalTick = 0; end
end
